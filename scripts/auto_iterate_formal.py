...
def _parse_report(report_dir, baseline_stem, agent_stem):
    ...
    return {
        ...
        "single_doc_f1": _find_task_f1(task_rows, agent_method, "single_doc_qa"),
        "multi_doc_f1": _find_task_f1(task_rows, agent_method, "multi_doc_qa"),
        "code_f1": _find_task_f1(task_rows, agent_method, "code_qa"),
...
def _score(can_m, dev_m, can_thresholds, holdout_thresholds):
    penalty_ratio = (
        0.20 * max(0.0, float(can_m["p95_ratio"]) - float(can_thresholds["p95_ratio"]))
        + 0.10 * max(0.0, float(dev_m["p95_ratio"]) - float(holdout_thresholds["p95_ratio"]))
    )
    penalty_oom = 0.05 * (int(can_m["agent_oom"]) + int(dev_m["agent_oom"]))
    return (
        0.60 * float(dev_m["agent_f1"])
        + 0.25 * float(can_m["agent_f1"])
        + 0.10 * float(dev_m["single_doc_f1"])
        + 0.05 * float(can_m["single_doc_f1"])
        - penalty_ratio
        - penalty_oom
    )
...
def main():
    ...
    while True:
        ...
        canonical = _run_agent_eval_single(
            py=py,
            config=args.config,
            dataset=args.canonical_dataset,
        ...
        canonical_ms = None
        holdout_ms = None
        trigger_multi = (single_pass_streak >= 2) or (rounds_since_multi >= int(args.force_multiseed_every))
        if trigger_multi:
            ...
            if canonical_ms["pass"]:
                ...
                holdout_last_pass = bool(holdout_ms["pass"])
            else:
                canonical_multi_pass_streak = 0
                holdout_last_pass = False
        ...
        print(
            "[round {}] can_f1={:.4f} can_single_doc={:.4f} can_ratio={:.4f} dev_f1={:.4f} "
            "gate_pass={} can_multi_streak={} holdout_last_pass={}".format(
                round_idx,
                canonical["agent_f1"],
        ...
        if canonical_multi_pass_streak >= 2 and holdout_last_pass:
            print("[stop] two consecutive canonical 3-seed passes and latest holdout pass.")
            break
